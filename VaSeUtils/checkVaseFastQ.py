...
def checkVaseFastQ(tmplFq1, vaseFq1, tmplFq2, vasesFq2, donorReadsFile, acceptorReadsFile):
	...
	if(checkVaseFastqSize(tmplFq1, vaseFq1, donorReads, acceptorReads)):
		print("VaSe produced FastQ R1 file " +str(vaseFq1)+ "seems to have the expected size.")
	else:
		print("VaSe produced FastQ R1 file " +str(vaseFq1)+ "does not seem to have the expected size.")
	
	# Check the VaSe produced R2 fastq file.
	if(checkVaseFastqSize(tmplFq2, vaseFq2, donorReads, acceptorReads)):
		print("VaSe produced FastQ R2 file " +str(vaseFq2)+ "seems to have the expected size.")
	else:
		print("VaSe produced FastQ R2 file " +str(vaseFq2)+ "does not seem to have the expected size.")
