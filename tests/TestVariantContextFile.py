...
class TestVariantContextFile(unittest.TestCase):
    def setUp(self):
        ...
        self.neg_otherVariantContextFile.add_variant_context('20_150', 'testsample2', '20', 150, 0, 350, self.var_context_d_reads_answer, self.var_context_a_reads_answer)
    ...
    def test_getAllVariantContextAcceptorReadIds(self):
        vca_read_ids = [self.va_read_id, self.va_read_id]
    ...
    def test_getAllVariantContextDonorReadIds(self):
        vcd_read_ids = [self.vd_read_id, self.vd_read_id]
    ...
    def test_variantIsInContext_pos(self):
        ...
        self.assertTrue(self.variantContextFile.variant_is_in_context(pos_variant_type, self.context_chrom_answer, self.context_origin_answer, self.context_origin_answer), f"The variant of type {pos_variant_type} on {self.context_origin_answer}, starting at {self.context_origin_answer} should have been in a context")
    
    def test_variantIsInContext_neg(self):
        neg_variant_type = 'aap'
        self.assertIsNone(self.variantContextFile.variant_is_in_context(neg_variant_type, self.context_chrom_answer, self.context_origin_answer, self.context_origin_answer), f"The variant of type {neg_variant_type} should have returned None")
    
    def test_snpVariantIsInContext_pos(self):
        self.assertTrue(self.variantContextFile.snp_variant_is_in_context(self.context_chrom_answer, self.context_origin_answer), f"The SNP on chromosome {self.context_chrom_answer} at position {self.context_origin_answer} should have been in a variant context")

    def test_snpVariantIsInContext_neg(self):
        neg_snppos = 325632
        self.assertFalse(self.variantContextFile.snp_variant_is_in_context(self.context_chrom_answer, neg_snppos), f"The SNP on chromosome {self.context_chrom_answer} at position {neg_snppos} should not have been in any variant context")
    
    def test_indelVariantIsInContext_pos(self):
        pos_indel_start = 9411050
        pos_indel_end = 9411150
        self.assertTrue(self.variantContextFile.indel_variant_is_in_context(self.context_chrom_answer, pos_indel_start, pos_indel_end), f"The indel on chromosome {self.context_chrom_answer}, starting at {pos_indel_start} and ending at {pos_indel_end} should have been in a variant context")
    
    def test_indelVariantIsInContext_neg(self):
        neg_indel_start = 8000000
        neg_indel_end = 8000100
        self.assertFalse(self.variantContextFile.indel_variant_is_in_context(self.context_chrom_answer, neg_indel_start, neg_indel_end), f"The indel on chromosome {self.context_chrom_answer}, starting at {neg_indel_start} and ending at {neg_indel_end} should not ahve been in any variant context")
    ...
    def test_addVariantContext(self):
        varcon_obj_answer = VariantContext('21_9411000', 'testanswer', '21', 9411000, 94110900, 9411100, self.var_context_a_reads_answer, self.var_context_d_reads_answer, self.setAccContextAnswer, self.setDonContextAnswer)
        self.variantContextFile.add_variant_context('21_9411000', 'testanswer', '21', 9411000, 94110900, 9411100, self.var_context_a_reads_answer, self.var_context_d_reads_answer, self.setAccContextAnswer, self.setDonContextAnswer)
        self.assertEqual(self.variantContextFile.get_variant_context('21_9411000').to_string(), varcon_obj_answer.to_string(), f"The obtained variant context for {self.context_id_answer} should have been the same as what was just added")
    ...
    def test_addAcceptorContext(self):
        self.variantContextFile.set_variant_context('21_9411000', self.setVarContextAnswer)
        acccon_obj_answer = OverlapContext('21_9411000', 'accanswer', '21', 9411000, 94110900, 9411050, self.acc_context_reads_answer)
        self.variantContextFile.add_acceptor_context('21_9411000', 'accanswer', '21', 9411000, 94110900, 9411050, self.acc_context_reads_answer)
    ...
    def test_addDonorContext(self):
        self.variantContextFile.set_variant_context('21_9411000', self.setVarContextAnswer)
        doncon_obj_answer = OverlapContext('21_9411000', 'donanswer', '21', 9411000, 94110950, 9411100, self.don_context_reads_answer)
        self.variantContextFile.add_donor_context('21_9411000', 'donanswer', '21', 9411000, 94110950, 9411100, self.don_context_reads_answer)
    ...
    def test_getVariantContextsSymmetricDifference_neg(self):
        neg_symdifference_answer = ['20_150', self.context_id_answer]
    ...
    def test_setAcceptorContextUnmappedMateIds(self):
        ...
        self.assertListEqual(self.variantContextFile.get_acceptor_context_unmapped_mate_ids(self.context_id_answer), acu_read_ids, f"The set and returned acceptor context unmapped mate ids should have been {acu_read_ids}")
    ...
    def test_setDonorContextUnmappedMateIds(self):
        ...
        self.assertListEqual(self.variantContextFile.get_donor_context_unmapped_mate_ids(self.context_id_answer), dcu_read_ids, f"The set and returned donor context unmapped mate ids should have been {dcu_read_ids}")
    ...
    def test_setUnmappedAcceptorMateIds(self):
        ...
        self.assertListEqual(self.variantContextFile.get_unmapped_acceptor_mate_ids(self.context_id_answer), vcau_read_ids, f"The set and returned unmapped acceptor mate ids should have been: {vcau_read_ids}")
    ...
    def test_setUnmappedDonorMateIds(self):
        ...
        self.assertListEqual(self.variantContextFile.get_unmapped_donor_mate_ids(self.context_id_answer), vcdu_read_ids, f"The set and returned unmapped donor mate ids should have been: {vcdu_read_ids}")
