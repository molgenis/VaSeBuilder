...
class TestVariantContextFile(unittest.TestCase):
    def setUp(self):
        ...
        self.variantContextFile.setVariantContext(self.contextIdAnswer, self.variantContextAnswer)
        self.variantContextFile.setAcceptorContext(self.contextIdAnswer, self.acceptorContextAnswer)
        self.variantContextFile.setDonorContext(self.contextIdAnswer, self.donorContextAnswer)
    ...
    def test_getAllVariantContextAcceptorReadIds(self):
        ...
        self.assertListEqual(self.variantContextFile.getAllVariantContextAcceptorReadIds(), vca_read_ids, f"The list of returned acceptor read ids should have been: {vca_read_ids}")

    def test_getAllVariantContextDonorReadIds(self):
        vcd_read_ids = [self.vdReadId, self.vdReadId]
        self.assertListEqual(self.variantContextFile.getAllVariantContextDonorReadIds(), vcd_read_ids, f"The list of returned donor read ids should have been: {vcd_read_ids}")
    ...
    def test_passesFilter_pos(self):
        pos_filter_val = 'aap'
        self.assertTrue(self.variantContextFile.passesFilter(pos_filter_val, self.filterListToUse), f"The value {pos_filter_val} should have been in the filter list {self.filterListToUse} and therefore return True")
        
    def test_passesFilter_neg(self):
        neg_filter_val = 'jan'
        self.assertFalse(self.variantContextFile.passesFilter(neg_filter_val, self.filterListToUse), f"The value {neg_filter_val} should not have been in the filter list {self.filterListToUse} and therefore return False")
    ...
    def test_variantIsInContext_pos(self):
        ...
        self.assertTrue(self.variantContextFile.variantIsInContext(pos_variant_type, self.contextChromAnswer, self.contextOriginAnswer, self.contextOriginAnswer), f"The variant of type {pos_variant_type} on {self.contextOriginAnswer}, starting at {self.contextOriginAnswer} should have been in a context")
    
    def test_variantIsInContext_neg(self):
        neg_variant_type = 'aap'
        self.assertIsNone(self.variantContextFile.variantIsInContext(neg_variant_type, self.contextChromAnswer, self.contextOriginAnswer, self.contextOriginAnswer), f"The variant of type {neg_variant_type} should have returned None")
    
    def test_snpVariantIsInContext_pos(self):
        self.assertTrue(self.variantContextFile.snpVariantIsInContext(self.contextChromAnswer, self.contextOriginAnswer), f"The SNP on chromosome {self.contextChromAnswer} at position {self.contextOriginAnswer} should have been in a variant context")

    def test_snpVariantIsInContext_neg(self):
        neg_snppos = 325632
        self.assertFalse(self.variantContextFile.snpVariantIsInContext(self.contextChromAnswer, neg_snppos), f"The SNP on chromosome {self.contextChromAnswer} at position {neg_snppos} should not have been in any variant context")
    
    def test_indelVariantIsInContext_pos(self):
        pos_indel_start = 9411050
        pos_indel_end = 9411150
        self.assertTrue(self.variantContextFile.indelVariantIsInContext(self.contextChromAnswer, pos_indel_start, pos_indel_end), f"The indel on chromosome {self.contextChromAnswer}, starting at {pos_indel_start} and ending at {pos_indel_end} should have been in a variant context")
    
    def test_indelVariantIsInContext_neg(self):
        neg_indel_start = 8000000
        neg_indel_end = 8000100
        self.assertFalse(self.variantContextFile.indelVariantIsInContext(self.contextChromAnswer, neg_indel_start, neg_indel_end), f"The indel on chromosome {self.contextChromAnswer}, starting at {neg_indel_start} and ending at {neg_indel_end} should not ahve been in any variant context")
    ...
    def test_setAcceptorContextUnmappedMateIds(self):
        ...
        self.variantContextFile.setAcceptorContextUnmappedMateIds(self.contextIdAnswer, acu_read_ids)
        self.assertListEqual(self.variantContextFile.getAcceptorContextUnmappedMateIds(self.contextIdAnswer), acu_read_ids, f"The set and returned acceptor context unmapped mate ids should have been {acu_read_ids}")

    def test_setDonorContextUnmappedMateIds(self):
        dcu_read_ids = ['dcuRead2', 'dcuRead4', 'dcuRead6']
        self.variantContextFile.setDonorContextUnmappedMateIds(self.contextIdAnswer, dcu_read_ids)
        self.assertListEqual(self.variantContextFile.getDonorContextUnmappedMateIds(self.contextIdAnswer), dcu_read_ids, f"The set and returned donor context unmapped mate ids should have been {dcu_read_ids}")

    def test_setUnmappedAcceptorMateIds(self):
        vcau_read_ids = ['vcauRead1', 'vcauRead3', 'vcauRead5']
        self.variantContextFile.setUnmappedAcceptorMateIds(self.contextIdAnswer, vcau_read_ids)
        self.assertListEqual(self.variantContextFile.getUnmappedAcceptorMateIds(self.contextIdAnswer), vcau_read_ids, f"The set and returned unmapped acceptor mate ids should have been: {vcau_read_ids}")

    def test_setUnmappedDonorMateIds(self):
        vcdu_read_ids = ['vcduRead2', 'vcduRead4', 'vcduRead6']
        self.variantContextFile.setUnmappedDonorMateIds(self.contextIdAnswer, vcdu_read_ids)
        self.assertListEqual(self.variantContextFile.getUnmappedDonorMateIds(self.contextIdAnswer), vcdu_read_ids, f"The set and returned unmapped donor mate ids should have been: {vcdu_read_ids}")
