...
class TestParamChecker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.param_list = {'vcfin': ['testdata/vcfDir'],
    ...
    def test_checkLog_pos(self):
        self.assertEqual(self.paramCheck.check_log("testdata/outDir/vaseutest.log"), "testdata/outDir/vaseutest.log",
                         "The log location should have been testdata/outDir/vaseutest.log")
    ...
    def test_checkLog_noParamSet(self):
        self.assertEqual(self.paramCheck.check_log(None), "VaSeBuilder.log", "The log location should have been VaSeBuilder.log")
    ...
    def test_checkLog_noLog(self):
        self.assertEqual(self.paramCheck.check_log("testdata/outDir/logfile.file"), "VaSeBuilder.log", "The log location should have been VaSeBuilder.log")
    ...
    def test_checkLog_noTxt(self):
        self.assertEqual(self.paramCheck.check_log("testdata/outDir/logfile.file"), "VaSeBuilder.log",
                         "The log location should have been VaSeBuilder.log")

    # Tests that the povided testdata folder 'bamDir' indeed contains two BAM files.
    def test_checkFolderContents_pos(self):
        self.assertEqual(self.paramCheck.check_folder_contents("testdata/bamDir", "bam"), 2,
    ...
    def test_checkFolderContents_neg(self):
        self.assertEqual(self.paramCheck.check_folder_contents("testdata/bamDir", "vcf"), 0,
    ...
    def test_checkFoldersExist_pos(self):
        self.assertListEqual(self.paramCheck.check_folders_exist(['testdata/bamDir'], 'bam'), ['testdata/bamDir'], "The folder should have been testdata/bamDir")

    # Tests that the provided testdata folder 'bamDir' exists and does not contain VCF files.
    def test_checkFoldersExist_neg(self):
        self.assertListEqual(self.paramCheck.check_folders_exist(['testdata/bamDir'], 'vcf'), [], "There should be no folder")

    # Tests that a provided file indeed exists.
    def test_checkFileExists_pos(self):
        self.assertTrue(self.paramCheck.check_file_exists("testdata/bamDir/SRR1039508.bam"), "File should exist")

    # Tests whether a non existing file indeed does not exist.
    def test_checkFileExists_neg(self):
        self.assertFalse(self.paramCheck.check_file_exists(""), "There should be no file")

    # Test that the output location for out.txt is valid.
    def test_isValidOutputLocation_pos(self):
        self.assertTrue(self.paramCheck.is_valid_output_location("testdata/outDir/out.txt"))

    # Test that the output location for out.txt is invalid.
    def test_isValidOutputLocation_neg(self):
        self.assertFalse(self.paramCheck.is_valid_output_location("testdata/doesnotexist/out.txt"))

    # Tests that the foldername a file is located in gets returned properly.
    def test_getFolderName_pos(self):
        self.assertEqual(self.paramCheck.get_folder_name("testdata/noSampleDir/noSampleBam.bam"), "testdata/noSampleDir", "Folder names should be equal bur are not")
    
    # Test that the foldername is empty.
    def test_getFolderName_neg(self):
        self.assertEqual(self.paramCheck.get_folder_name(""), "", "Folder names should both be empty.")
    ...
    def test_checkParameters_pos(self):
        self.assertTrue(self.paramCheck.check_parameters(self.param_list))

    def test_checkParameters_noVcfIn(self):
        parList = self.param_list.copy()
        parList['vcfin'] = ['testdata/doesnotexist']    # Set the list VCF folders to a folder that does not exist.
        self.assertFalse(self.paramCheck.check_parameters(parList))
