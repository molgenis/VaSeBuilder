...
class OverlapContext:
	...
	def __init__(self, variantid, sampleid, ovconchrom, ovconorigin, ovconstart, ovconend, bamreads):
	...
	def getContextBamReadLeftPositions(self):
		return [x.getBamReadRefPos() for x in self.contextBamReads if(x.isRead1())]
	...
	def compare(self, otherOverlapContext):
		...
		if(self.contextId != otherOverlapContext.getContextId()):
			differences[1] = [self.contextId, otherOverlapContext.getContextId()]
		if(self.sampleId != otherOverlapContext.getSampleId()):
			differences[2] = [self.sampleId, otherOverlapContext.getSampleId()]
		if(self.contextChrom != otherOverlapContext.getContextChrom()):
			differences[3] = [self.contextChrom, otherOverlapContext.getContextChrom()]
		if(self.contextOrigin != otherOverlapContext.getContextOrigin()):
			differences[4] = [self.contextOrigin, otherOverlapContext.getContextOrigin()]
		if(self.contextStart != otherOverlapContext.getContextStart()):
			differences[5] = [self.contextStart, otherOverlapContext.getContextStart()]
		if(self.contextEnd != otherOverlapContext.getContextEnd()):
			differences[6] = [self.contextEnd, otherOverlapContext.getContextEnd()]
		if(self.getContextBamReadIds().sort() != otherOverlapContext.getContextBamReadIds().sort()):
			differences[7] = [self.contextBamReads, otherOverlapContext.getContextBamReads()]
		return differences
